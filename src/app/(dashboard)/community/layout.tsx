...
const NAV_ITEMS = [
  { href: "/community", label: "Feed", icon: LayoutDashboard },
  { href: "/community/surplus", label: "Surplus", icon: Leaf },
  { href: "/community/leftovers", label: "Leftovers", icon: Flame },
  { href: "/community/kitchen", label: "Kitchen", icon: Users },
  { href: "/community/leaderboard", label: "Leaderboard", icon: Trophy },
  { href: "/community/impact", label: "Impact", icon: Activity },
        ...
        <nav className="flex-1 px-4 py-6 space-y-1">
          {NAV_ITEMS.map((item) => {
            ...
            const isActive =
              mounted &&
              pathname &&
              (item.href === "/community" ? pathname === "/community" : pathname?.startsWith(item.href));
            return (
              <Link
                key={item.href}
                href={item.href}
...
function CommunityTopNav() {
  ...
  const xp = useMemo(() => ({ current: 320, next: 500, level: 4 }), []);
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end" className="w-56" sideOffset={8}>
              <DropdownMenuLabel>
                <div className="flex items-center gap-3 py-2">
                  ...
                  <div className="space-y-0.5">
                    <p className="text-sm font-medium leading-none">{user?.name ?? "Community Member"}</p>
              ...
              <DropdownMenuItem asChild className="focus:bg-emerald-50 focus:text-emerald-600">
                <Link href="/community/profile" className="flex items-center gap-2">
